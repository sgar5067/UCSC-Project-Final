...
def reorder_nodes(nxgraph, method, existing_mapping, node_feat_name):
    """
    ...
    :param nxgraph:
    :param method: 'bfs', 'degree', 'existing', None.
    :param existing_mapping: Only used when method == 'existing'.
    ...
    if logging_enabled == True:
        print("- Entered node_ordering::reorder_nodes Public Method")
...
def _bfs_seq(graph, start_id, node_feat_name, sort_ignore_nodes=[]):
    ...
    if logging_enabled == True:
        print("- Entered node_ordering::reorder_nodes Private Method")
...
def _sorted_nodes_based_on_node_deg_and_types(nodes, graph, node_feat_name):
    ...
    if (type(graph) == nx.Graph) or (type(graph) == nx.MultiGraph):
    ...
    if node_feat_name:
        ...
        decorated = [(degree_fn()[node], types[node], node) for node in nodes]
